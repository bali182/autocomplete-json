...
export const updateSchema = node => schema => {
  // mutation, not pretty
  assign(node, schema)
  delete schema.$ref
}
...
const resolveDocument = (root, node) => {
  ...
  return uri.reference === 'same-document'
    ? Promise.resolve(updateSchema(node)(resolveInSameDocument(node, $ref.split('/'))))
...
const traverseResolve = root => node => (node.$ref ? resolveDocument(root, node) : Promise.resolve()).then(() => {
  const childNodes = findChildNodes(root, node)
  const childResolvePromises = childNodes.map(traverseResolve(root))
